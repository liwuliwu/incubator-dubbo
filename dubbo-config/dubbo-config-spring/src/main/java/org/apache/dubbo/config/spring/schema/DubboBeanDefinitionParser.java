    ...
    private static void parseNested(Element element, ParserContext parserContext, Class<?> beanClass, boolean required, String tag, String property, String ref, BeanDefinition beanDefinition) {
        ...
        if (nodeList != null && nodeList.getLength() > 0) {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node instanceof Element) {//只解析指定标签。目前有内嵌的 <dubbo:service /> 和 <dubbo:reference /> 标签。
                    if (tag.equals(node.getNodeName())
                        ...
                        BeanDefinition subDefinition = parse((Element) node, parserContext, beanClass, required);//解析指定标签，创建子 Bean 对象。
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    if ("parameter".equals(node.getNodeName())
                        ...
                        String value = ((Element) node).getAttribute("value");
                        boolean hide = "true".equals(((Element) node).getAttribute("hide"));//<dubbo:parameter hide=“” /> 的用途
                        if (hide) {
